...
export function Sidebar() {
  ...
  function handleReset() {
    if (!window.confirm("Reset all demo data to its original state? This clears every change made in this session.")) {
      return;
    }
    resetDemoData();
    ...
    <aside className="hidden md:flex h-screen w-64 flex-col border-r border-sidebar-border bg-sidebar shrink-0 sticky top-0">
      <div className="flex items-center gap-2.5 px-5 py-5 border-b border-sidebar-border">
        <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-primary/15 border border-primary/30">
          <Gem className="h-4.5 w-4.5 text-primary" />
        </div>
        <div className="leading-tight">
          <p className="text-sm font-semibold text-foreground tracking-tight">
            Bharat Tech Verse
          </p>
          <p className="text-[11px] text-muted-foreground">CRM Suite</p>
        </div>
      </div>
